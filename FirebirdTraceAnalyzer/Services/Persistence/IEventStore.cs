...
public interface IEventStoreReader
{
    ...
    /// <summary>
    /// Стрим событий по диапазону времени (без загрузки всего в память).
    /// <para>
    /// КОНТРАКТ ПЕРЕЧИСЛЕНИЯ: держит открытый reader на общем соединении стора и отдаёт события лениво.
    /// Перечисление ДОЛЖНО полностью завершиться в рамках ОДНОГО обращения к диспетчеру
    /// (<c>EventStoreDispatcher</c>), до любой другой операции со стором — иначе параллельная команда
    /// на том же соединении конфликтует с активным reader (у SQLite один reader на соединение).
    /// Не сохранять ленивый результат наружу и не перемежать с другими вызовами стора.
    /// </para>
    /// </summary>
