...
RETURNING seq;");
    ...
    public void ExportTo(string targetDbPath, IEnumerable<TraceFileInfoModel> files)
    {
        ...
        // BEST-EFFORT: это строковое сравнение, а не проверка тождества файла — оно НЕ покрывает
        // симлинки/хардлинки и регистронезависимые тома, смонтированные под Linux (там case-вариант
        // пути укажет на тот же файл, но гвард не сработает). Точная проверка потребовала бы сравнения
        // device+inode (непортируемо); для типовых путей строкового сравнения достаточно.
